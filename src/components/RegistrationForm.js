...
// Stepper Context
export const StepperContext1 = createContext();

// Account Component
export function Account() {
  const { userData, setUserData } = useContext(StepperContext1);
  ...
  return (
    <div className='w-fill h-fill flex flex-col'>
      <div className="w-fill mx-2 flex-1">
  ...
  const displaySteps = newStep.map((step, index) => (
    <div key={index} className={index === newStep.length - 1 ? "w-full flex items-center" : "flex items-center"}>
      <div className="relative flex flex-col items-center text-teal-600">
        <div className={`rounded-full transition duration-500 ease-in-out border-2 border-gray-300 h-12 w-12 flex item-center justify-center py-3 ${step.selected ? "bg-green-600 text-white font-bold border-green-600" : ""}`}>
          {step.completed ? (
    ...
    <div className="md:w-1/2 mx-auto shadow-xl rounded-2xl pb-2 bg-white">
      ...
      <div className="container horizontal mt-5">
        ...
        <div className="my-10 p-10">
          <StepperContext1.Provider value={{
            userData,
            setUserData,
            finalData,
            setFinalData,
